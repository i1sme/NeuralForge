.globl _nfl_forward_SoftmaxWithBias
.p2align 2
_nfl_forward_SoftmaxWithBias:
    stp     x19, x20, [sp, #-16]!
    stp     x21, x22, [sp, #-16]!
    str     x23, [sp, #-16]!
    stp     d8, d9, [sp, #-16]!
    ...
    mov     x29, sp
    sub     sp, sp, #256
    ; matmul: input [4,8] x weights [8,16] -> output [4,16] + fused
    fmov    s4, wzr
    mov     x11, x0
    mov     x12, sp
    mov     x13, x1
...
.Lmm_j_end_0_0:
    ...
    b       .Lmm_i_0_0
.Lmm_i_end_0_0:
    ; matmul: input [4,16] x weights [16,3] -> output [4,3] + bias + fused
    mov     x11, sp
    mov     x12, x2
    movz    x9, #0x0080
    add     x13, x1, x9, lsl #2
...
.Lmm_j_end_0_1:
    ...
    b       .Lmm_i_0_1
.Lmm_i_end_0_1:
    ; fused softmax_row: output [4,3] in-place
    mov     x22, x12
    mov     x23, x12
    mov     x19, #0
.Lfsmx_i_0_1:
